...
namespace ETraffic
{
    public class QRGenerator
    {
        ...
        public QRCodeData StringToQRBin(string plainText)
        {
            //encoding EncodingMode.Alphanumeric
            // , utf8BOM false, forceUtf8 false)
            // requestVersion -1 EciMode.Default
            var codedText = this.PlainTextToBinaryAlphanumeric(plainText);
            var dataInputLength = plainText.Length;
            var version = this.GetVersion(dataInputLength);

            string modeIndicator = DecToBin(2, 4); /* Encoding.Arifmetic */

            var countIndicator = DecToBin(dataInputLength, this.GetCountIndicatorLength(version));
            var bitString = modeIndicator + countIndicator;
            bitString += codedText;

            //Fill up data code word
            var eccInfo = this.capacityECCTable.Single(x => x.Version == version);
            ...
            var formatStr = GetFormatString(maskVersion);
        ...
        private static string GetFormatString(int maskVersion)
        {
        ...
        private int GetVersion(int length)
        {
            var version = this.capacityTable.Where(
                x => x.Capacity >= Convert.ToInt32(length)
              ).Min(x => x.Version);
            return version;
        }
        private void CreateCapacityTable()
        {
            ...
            {
                this.capacityTable.Add(new VersionInfo((i + 16) / 16, this.capacityBaseValues[i+1]));
            }
        }
        ...
        private int GetCountIndicatorLength(int version)
        {
            if (version < 10)
            {
                return 9;
        ...
        {
            public VersionInfo(int version, int capacity)
            {
                this.Version = version;
            ...
            public static int MaskCode(ref QRCodeData qrCode, int version, ref List<Rectangle> blockedModules)
            {
                ...
                foreach (var pattern in methods)
                {
                    if (pattern.Name.Length == 8 && pattern.Name.StartsWith("Pattern"))
                    {
                        ...
                        var formatStr = GetFormatString(Convert.ToInt32((pattern.Name.Substring(7, 1))) - 1);
